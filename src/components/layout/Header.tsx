...
import { Bell, Compass, Radio, Search, Shield, Trophy, Users } from 'lucide-react'
    ...
    <header className="sticky top-0 z-40 bg-[#F5F6F8]/92 backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-[1440px] items-center justify-between px-4 py-4 sm:px-5 lg:px-6">
        ...
        <nav className="hidden items-center gap-8 text-xs font-black uppercase tracking-wide text-[#111827] lg:flex">
          ...
          <Link href="/ranking" className="pb-2">Ao vivo</Link>
          <Link href="/ranking" className="pb-2">Ranking</Link>
          <Link href="/amigos" className="pb-2">Amigos</Link>
          <Link href="/amigos" className="pb-2">Explorar</Link>
          <Link href="/premium" className="pb-2">Recompensas</Link>
        </nav>
        ...
        <nav className="ml-3 hidden flex-1 justify-center gap-2 sm:flex lg:hidden">
          {[
            ...
            <Link key={label} href={index === 2 ? '/amigos' : index === 1 ? '/ranking' : '/'} className={`flex min-w-[64px] flex-col items-center gap-1 rounded-2xl px-3 py-2 text-[10px] font-bold ${index === 0 ? 'bg-[#6C3BFF]/10 text-[#6C3BFF] shadow-sm' : 'text-[#111827]'}`}>
          ...
          {!profile ? (
            ...
            <Link href="/perfil" className="flex items-center gap-3">
              ...
              <div className="hidden leading-tight lg:block">
                ...
                <div className="text-[11px] font-black text-[#16C45B]">⊕ 1.250</div>
              </div>
            </Link>
          )}
        </div>
